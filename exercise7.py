...
def main():
    ...
    while True:
        user_input = int(input("Enter a number of a month from(1-12): "))
        if user_input == "" and user_input<1 and user_input>12:
            break
        elif 0<user_input<=2 or user_input==12:
            print(season[3])
        elif 3<=user_input<=5:
            print(season[0])
        elif 6<=user_input<=8:
...
#7.2 Write a program that asks the user to enter names until he/she enters an empty string.
# After each name is read the program either prints out New name or Existing name depending on
# whether the name was entered for the first time.
...
#7.3 Write a program for fetching and storing airport data.
# The program asks the user if they want to enter a new airport, fetch the information of an existing
# airport or quit. If the user chooses to enter a new airport, the program asks the user to enter
...
# If the user chooses to quit, the program execution ends. The user can choose a new option as many
# times they want until they choose to quit.
